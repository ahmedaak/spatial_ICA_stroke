...
# HBM Revision #1 
# exclude follow-up scans from training + testing of prediction algorithm
# get follow-up dataset numbers
FU_nums <- c(29,31,37,57,59,128,146,148,154,160,169,175,184,187)
HFE_full_withfu <- HFE_full
HFE_full <- HFE_full[ ! HFE_full$subject %in% FU_nums, ]

# HBM Revision #1 - train prediction algorithm on FIX training data
# get FIX training dataset numbers 
FIX_training_nums <- c(5,13,19,30,36,47,56,58,67,106,127,130,145,147,153,159,165,168,171,174)
HFE_full_training <- HFE_full[ HFE_full$subject %in% FIX_training_nums, ]
HFE_full_test <- HFE_full[ ! HFE_full$subject %in% FIX_training_nums, ]

# CREATE BALANCED DATASET + TRAIN MODEL ON IT
# select the features of rows corresponding to HICs
HFE_full_HIC <- HFE_full_training[HFE_full_training$HIC==1,]
# select the features of rows corresponding to non-HICs
HFE_full_nonHIC <- HFE_full_training[HFE_full_training$HIC==0,]
...
lasso_HFE_crossval <- cv.glmnet(x = as.matrix(HFE_full_balanced_subsamp[,-excl_var]), nfolds = nrow(HFE_full_balanced_subsamp), y = as.matrix(HFE_full_balanced_subsamp[,3]), family = "binomial", alpha = 0.5, type.measure = "mse", grouped = FALSE) 
...
for (i in 1:50) {
  # CREATE UNBALANCED (REALISTIC) TEST DATASET + VALIDATE MODEL ON IT
    # select a sample of HICs (indices) that were NOT included in the training dataset (n=5 out of possible 15)
      ...
      HFE_feats_unbalanced_test_HIC <- HFE_full_test[HFE_full_test$HIC==1,]
      # get all non-HICS in test data
      HFE_feats_unbalanced_test_nonHIC <- HFE_full_test[HFE_full_test$HIC==0,]
      # get sample of all HICS in test data (5 out of possible 22)
    ...
    # select a sample of non-HICs equal to 10*n of HICs (indices, then features) that were NOT included in training
      nonhic_feats_sampleindices_test <- sample(x = nrow(HFE_feats_unbalanced_test_nonHIC), size = 10*length(HFE_feats_unbalanced_test_HICsample), replace = FALSE)
      nonhic_feats_sample_test <- HFE_feats_unbalanced_test_nonHIC[nonhic_feats_sampleindices_test,]
    # combine the HICs + nonHICs features
...
pdf(file = "../figures/figure7/Figure7.pdf", width = 13, height = 6)
...
HFE_HIC <- HFE_full_withfu[HFE_full_withfu$HIC==1,]
HFE_full_HIC_mean <- colMeans(HFE_HIC[,4:13],na.rm=T)
HFE_full_HIC_95ci_ul <- HFE_full_HIC_mean + 1.96*(apply(HFE_HIC[,4:13],2,sd)/sqrt(nrow(HFE_HIC)))
names(HFE_full_HIC_95ci_ul) <- paste(names(HFE_full_HIC_95ci_ul),"_95%CI_ul",sep="")
HFE_full_HIC_95ci_ll <- HFE_full_HIC_mean - 1.96*(apply(HFE_HIC[,4:13],2,sd)/sqrt(nrow(HFE_HIC)))
names(HFE_full_HIC_95ci_ll) <- paste(names(HFE_full_HIC_95ci_ll),"_95%CI_ll",sep="")

HFE_nonHIC <- HFE_full_withfu[HFE_full_withfu$HIC==0,]
HFE_full_nonHIC_mean <- colMeans(HFE_nonHIC[,4:13],na.rm=T)
HFE_full_nonHIC_95ci_ul <- HFE_full_nonHIC_mean + 1.96*(apply(HFE_nonHIC[,4:13],2,sd)/sqrt(nrow(HFE_nonHIC)))
names(HFE_full_nonHIC_95ci_ul) <- paste(names(HFE_full_nonHIC_95ci_ul),"_95%CI_ul",sep="")
HFE_full_nonHIC_95ci_ll <- HFE_full_nonHIC_mean - 1.96*(apply(HFE_nonHIC[,4:13],2,sd)/sqrt(nrow(HFE_nonHIC)))
...
epi_temp <- readNIfTI("../figures/figure3/AK_103_EPI_template_brain.nii.gz")
dwi_bl <- readNIfTI("../figures/figure3/147/dwi2epiWarped.nii.gz")
dwi_fu <- readNIfTI("../figures/figure3/148/dwi2epiWarped.nii.gz")
tmax_bl <- readNIfTI("../figures/figure3/147/Tmax_sSVD_norm.nii.gz")
tmax_fu <- readNIfTI("../figures/figure3/148/Tmax_norm.nii.gz")
IC_bl <- intersect(list.files("../figures/figure3/147/", pattern = "thresh_"), list.files("../figures/figure3/147/", pattern = ".nii.gz$"))
IC_fu <- intersect(list.files("../figures/figure3/148/", pattern = "thresh_"), list.files("../figures/figure3/148/", pattern = ".nii.gz$"))
...
for (f in c("epi_temp", "dwi_bl", "dwi_fu", "tmax_bl", "tmax_fu")){
  ...
  dev.off()
  # plot mosaics
  png(paste("../figures/figure3/",f,"_mosaic.png",sep=""))
  if (f == "tmax_bl" | f == "tmax_fu"){
...
for (g in c(IC_bl)){
  # load in IC NIFTI
  ic <- readNIfTI(paste("../figures/figure3/147/",g,sep=""))
  # plot single slice
  png(paste("../figures/figure3/147/",g,".png",sep=""))
  overlay(epi_temp, ifelse(ic > 0, ic, NA), z = 40, plot.type = "single", col.y=jet.colors(7), zlim.y=c(0,10))
  dev.off()
  # plot mosaic
  png(paste("../figures/figure3/147/",g,"_mosaic.png",sep=""))
...
for (h in c(IC_fu)){
  # load in IC NIFTI
  ic <- readNIfTI(paste("../figures/figure3/148/",h,sep=""))
  # plot single slice
  png(paste("../figures/figure3/148/",h,".png",sep=""))
  overlay(epi_temp, ifelse(ic > 0, ic, NA), z = 40, plot.type = "single", col.y=fire.colors(15), zlim.y=c(0,10))
  dev.off()
  # plot mosaic
  png(paste("../figures/figure3/148/",h,"_mosaic.png",sep=""))
