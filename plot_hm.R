...
pwi_hm <- read.csv("13_pwi_hm.txt",header=F)
rs_hm <- read.csv("13_rs_hm.txt", header=F)

# plot motion
png("13_pwi_hm.png", width = 15, height = 8, units = "cm", res=600)
plot(seq(from=1,to=nrow(pwi_hm)*1.39,by=1.39),pwi_hm$V1, type="l",lwd=3,xlab="Time (s)",ylab="Displacement (mm)",ylim=c(0,3.5))
dev.off()

pdf("13_rs_hm.pdf", width = 6, height = 3.5)
